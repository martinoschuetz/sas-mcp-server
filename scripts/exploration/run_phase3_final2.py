...
async def main():
    ...
    # We must use PART.REPL_PART_CD for Failure Relationships because that's what it's built for!
    await run_failure(
        name=f"Phase 3 - Failure Rel (Alarm 1) {uid}",
        data_selection_id=child_ds_id,
        folder_id=folder_id,
        analysis_var="PART.REPL_PART_AMT",
        report_var="PART.REPL_PART_CD",
        data_domain="PRODUCT,CLAIM,PART",
        rv_dim_column="PART.REPL_PART_CD",
        rv_table_name_key="PART.REPL_PART_CD",
        rv_table_name_value="PART.REPL_PART_CD",
        rv_table_name="PART.REPL_PART_CD",
        ctx=ctx,
