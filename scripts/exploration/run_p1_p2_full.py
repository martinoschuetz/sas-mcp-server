...
async def main():
    ...
    report_var = 'PRODUCT.SELLING_DEALER_COUNTRY_CD,PRODUCT.CSTMR_STATE_CD,PRODUCT.PRODUCTION_MONTH,PRODUCT.MODEL_CD,PRODUCT.PLANT_CD,CLAIM.EVENT_TYPE_CD,PRODUCT.SELLING_DEALER_CD,PRODUCT.CSTMR_COUNTRY_CD'
    
    print('Dispatching Full Pareto...')
    await run_pareto_analysis(
        name='Phase 1 - Pareto Alert 2 Full',
    ...
    print('Dispatching Full Stat Driver...')
    await run_statistical_driver_analysis(
        name='Phase 2 - Stat Driver Alert 2 Full',
        data_selection_id=ds_id,
        folder_id=folder_id,
        ctx=ctx,
        report_var=report_var,
