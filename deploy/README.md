...
**3. Image pull.** The manifests use `ghcr.io/sassoftware/sas-mcp-server:1.16.1`.
If the cluster cannot pull from ghcr.io, mirror it first:

```sh
podman pull ghcr.io/sassoftware/sas-mcp-server:1.16.1
podman tag ghcr.io/sassoftware/sas-mcp-server:1.16.1 \
  registry.example.com/library/sas-mcp-server:1.16.1
podman push registry.example.com/library/sas-mcp-server:1.16.1
```
